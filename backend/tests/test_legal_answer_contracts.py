...
def test_factory_generates_all_twenty_one_contracts_in_three_languages():
    cases = build_tax_answer_contract_cases()

    assert len(TAX_FAQ_MATRIX) == 21
    assert len(cases) == 63
    ...
    assert len({case["id"] for case in cases}) == 63
...
def test_final_boundary_enforces_all_sixty_three_citations_idempotently():
    for contract in TAX_FAQ_MATRIX:
        for language in SUPPORTED_LANGUAGES:
...
def test_contract_audit_is_complete_read_only_and_call_free():
    ...
    assert report["contracts"] == 21
    assert report["localized_answers"] == 63
    assert report["generated_cases"] == 63
    assert report["verified_article_bindings"] == 21
...
def test_factory_builds_a_valid_sixty_three_case_public_canary():
    suite = build_suite()

    assert validate_suite_payload(suite) is suite
    assert suite["suite_version"].startswith("legal-answer-contracts-")
    assert suite["execution_profile"]["max_public_requests"] == 63
