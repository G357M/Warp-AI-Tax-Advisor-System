...
stored ``receiptDate`` metadata, and ``document_number`` from ``documentNumber``
(the exact_doc_resolver dispute lookup matches on that column).
...
def main() -> None:
    ...
    try:
        ...
        for doc in rows:
            ...
            if doc.document_type not in CANONICAL_TYPES:
            ...
            if doc.date_published is None:
                ...
                if parsed:
                    ...
                    dates_filled += 1
            if doc.document_number is None:
                number = str(meta.get("documentNumber") or "").strip()
                if number:
                    doc.document_number = number
                    numbers_filled += 1

        print(f"Rows selected for backfill: {len(rows)}")
        ...
        print(f"document_number filled from documentNumber: {numbers_filled}")
