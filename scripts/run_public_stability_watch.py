...
PROFILE_QUERY_FILES = {
    ...
    'multilingual-core': 'scripts/public_canary_queries_multilingual_core.txt',
    'multilingual-trimmed': 'scripts/public_canary_queries_multilingual_trimmed.txt',
}
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--profile', choices=['core', 'extended', 'trimmed', 'multilingual-core', 'multilingual-trimmed'])
...
def _parse_query_line(line: str, default_language: str) -> Dict[str, str]:
    raw = line.strip()
    if not raw:
        raise ValueError('empty query line')

    lang_match = re.match(r'^\[(ru|en|ka)\]\s*(.+)$', raw, re.IGNORECASE)
    if lang_match:
        return {'language': lang_match.group(1).lower(), 'query': lang_match.group(2).strip()}

    if '\t' in raw:
        maybe_lang, query = raw.split('\t', 1)
        maybe_lang = maybe_lang.strip().lower()
        if maybe_lang in {'ru', 'en', 'ka'} and query.strip():
            return {'language': maybe_lang, 'query': query.strip()}

    return {'language': default_language, 'query': raw}


def _load_queries(path: Optional[str], default_language: str) -> List[Dict[str, str]]:
    if not path:
        return [{'language': default_language, 'query': query} for query in DEFAULT_QUERIES]
    with open(path, 'r', encoding='utf-8') as fh:
        return [
            _parse_query_line(line, default_language)
            for line in fh
...
def _language_mismatch(language: str, text: str) -> bool:
    if not text:
        return False
    cleaned = re.sub(r"(?im)^\s*(?:source|источник|წყარო):.*$", "", text)
    cleaned = re.sub(r"\n{2,}", "\n", cleaned).strip()
    cyrillic_chars = len(re.findall(r"[А-Яа-яЁё]", cleaned))
    georgian_chars = len(re.findall(r"[\u10A0-\u10FF]", cleaned))
    latin_letters = len(re.findall(r"[A-Za-z]", cleaned))

    if language == 'en':
        return cyrillic_chars >= 8 or georgian_chars >= 8
    if language == 'ka':
        return cyrillic_chars >= 8 or (georgian_chars == 0 and latin_letters > 40)
    if language == 'ru':
        return georgian_chars >= 8 and cyrillic_chars < 8
    return False
...
def _classify_response(query: str, language: str, attempt: int, result: Dict[str, Any], client_profile: str) -> Dict[str, Any]:
    ...
    language_mismatch = _language_mismatch(language, response_text)
...
def main() -> int:
    args = _parse_args()
    queries = _load_queries(_resolve_queries_path(args.queries_file, args.profile), args.language)
    rows: List[Dict[str, Any]] = []
    user_agent, origin, referer, respect_rate_limit, client_profile = _resolve_request_profile(args)

    for query_item in queries:
        query = query_item['query']
        query_language = query_item['language']
        for attempt in range(1, args.repeats + 1):
            result = _post_json(
                args.url,
                {'query': query, 'language': query_language},
                timeout=args.timeout,
                user_agent=user_agent,
                origin=origin,
                referer=referer,
            )
            row = _classify_response(query, query_language, attempt, result, client_profile)
            rows.append(row)
    ...
    summary = []
    for query_item in queries:
        label = f"[{query_item['language']}] {query_item['query']}"
        items = [row for row in rows if row['query_label'] == label]
        ok_items = [row for row in items if row['http_status'] == 200]
        latencies = [row['elapsed_s'] for row in ok_items]
        summary.append({
            'query': query_item['query'],
            'request_language': query_item['language'],
            'query_label': label,
            'attempts': len(items),
