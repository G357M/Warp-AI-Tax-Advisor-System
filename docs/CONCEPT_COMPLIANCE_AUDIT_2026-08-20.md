...
| Проверяемая поставка | **Сильное соответствие после модернизации** | Next 16.3.1 / React 19.2.8, реальные backend contracts, dependency audit и Docker builds в CI; ручной CD с pinned host key; CPU-only ML runtime; cache-only embedding startup с health/deploy audit; scoped rollback retention; отдельный bounded `docker-container` builder, nightly read-only disk/cache-pressure audit и SHA-pinned exact-ID legacy-retirement tool | Первый reviewed exact-ID retirement ещё требует выполнения и post-check; автоматический global prune запрещён |
...
2. Выполнить первым reviewed batch отдельный SHA-pinned exact-ID retirement четырёх атрибутированных старых InfoHub dependency records в host-wide `default` builder и сохранить post-check; новый builder изолирован, а nightly audit лишь наблюдает пороги и никогда не выполняет global prune.
