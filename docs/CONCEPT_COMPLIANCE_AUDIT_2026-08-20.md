...
До полного продуктового соответствия остаются не декоративные, а системные функции: production-доставка писем для уже реализованного восстановления доступа, автоматизированная коммерция, организации Business и постоянно измеряемое качество RAG. Поэтому общий вердикт — **сильная рабочая основа, частично завершённая продуктовая экосистема**.
...
| Проверяемая поставка | **Сильное соответствие после модернизации** | Next 16.3.1 / React 19.2.8, реальные backend contracts, dependency audit и Docker builds в CI; ручной CD с pinned host key; CPU-only ML runtime; cache-only embedding startup с health/deploy audit; scoped rollback retention; отдельный bounded `docker-container` builder, nightly read-only disk/cache-pressure audit и завершённый SHA-pinned exact-ID legacy retirement с post-check | Автоматический global prune по-прежнему запрещён; новый legacy cache требует отдельной повторной атрибуции |
...
В CI уже добавлен версионируемый 17-case offline golden set по RU/EN/KA
нормативным, локальным и dispute-маршрутам. Он блокирует изменение при деградации
classification accuracy, top-1 locator recall, source-audit accuracy или exact
citation rate; дополнительно запускаются 117 существующих RAG v2 тестов и 113
matrix subtests. Контракт отдельно фиксирует распространённые формы `НК`,
`Art.` / `No.` и parenthetical point notation на RU/EN/KA, не принимая обычное
английское отрицание `no` перед числом за номер документа или `part` за статью. Этот слой
детерминирован и не выдаётся за проверку живого корпуса.
...
Он автоматически сравнивает текущие нормализованные тексты, порядок токенов,
структурированный результат и реквизиты, сохраняя только fingerprints,
similarity и metadata differences. Точное совпадение становится кандидатом для
пакетного подтверждения, а консервативный high-overlap сигнал попадает в
отдельную короткую очередь без автоматического выбора canonical-версии.
...
2. Сохранять shared `default` builder в observe-only режиме: первый reviewed exact-ID graph retirement завершён и подтверждён post-check, а любой будущий batch требует новой атрибуции и никогда не заменяется global prune.
