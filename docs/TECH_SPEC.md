...
# Техническое задание: tax-advisor.ge / InfoHub

## 1. Общая информация о проекте

### 1.1 Наименование проекта
**tax-advisor.ge / InfoHub** — AI-система для работы с грузинским налоговым и смежным правовым корпусом

### 1.2 Цель проекта
Создание AI-системы, которая автоматически собирает, обрабатывает и предоставляет информацию по налоговому законодательству Грузии из корпуса infohub.rs.ge / infohubapi.rs.ge с использованием технологий искусственного интеллекта.

### 1.3 Основные задачи
1. Автоматический сбор документов с сайта infohub.rs.ge
...
- Автоматический обход сайта infohub.rs.ge
...
#### 2.3.2 Векторное хранилище (pgvector в PostgreSQL)
- Хранение embeddings в PostgreSQL с расширением pgvector
- Vector similarity search поверх pgvector
- Фильтрация по метаданным и SQL-полям
- Cosine distance / similarity для retrieval
...
| Изменение структуры infohub.rs.ge / native API | Средняя | Высокое | Мониторинг, быстрое обновление парсеров и экспортёров |
...
- ✓ Система собирает релевантные документы с infohub.rs.ge / native API
