...
35. **Parser-backed legal answers now share one scalable article contract.** The factory contains 28 expert-authored contracts / 84 RU/EN/KA answers and 36 verified article bindings. Tax residency, late-payment interest, tax appeals, LLC small-business ineligibility, import VAT, the VAT-registration threshold, inbound tour-operator VAT treatment, the distributed-profit model and property-tax guidance no longer keep independent answer text in legacy guards. The same contract now controls direct routing, the primary retrieval article, the final citation boundary and the generated public canary, preventing answer/retrieval citation drift. Property-tax guidance was corrected to Tax Code articles 202 and 206: it distinguishes the company cap, both individual income bands and the GEL 40,000 non-land exemption instead of incorrectly citing penalty article 281. A syntax-derived CI audit classifies all 11 parser goals: seven are contract-backed and four require contextual retrieval; it also rejects any new hard-coded authoritative fact outside the one explicit funded-pension exception pending a verified external-law provision registry. Contract and coverage audits are read-only and call-free; the generated 84-request suite remains dry-run-first, loopback-only and manually executed because public responses may invoke the provider.
