...
34. **Exact article links are verified at the final public-response boundary.** A separate dry-run-first canary permits exactly three loopback HTTP requests to `/api/v1/public/query`, one per RU/EN/KA, and refuses external targets or a different request ceiling. It requires a correctly localized article-47 answer, `exact_provision` evidence, the canonical Matsne `#part_173` link and the separately pinned Labour Code publication 28 URL in the serialized public source metadata. The accepted production run for `ceb7ac7` passed 3/3 with all seven overall and per-language metrics at 1.0. Full answers and sources remain in a mode-600 operational report; only the aggregate allowlist is committed. Because public requests may invoke the provider, this canary remains manual and is not added to nightly automation.
