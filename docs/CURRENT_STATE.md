...
11. **Rollback and future build-cache growth are bounded.** Для backend/frontend сохраняются три новейших main-branch rollback-тега. Production images собираются отдельным `infohub-production-v1` builder с `docker-container` driver; после build только его cache ограничивается 18 GB при 6 GB reserved и 25 GB min-free. Активный host-wide `default` builder, volumes, неизвестные теги и cache других проектов не очищаются.
...
- отдельная ручная инвентаризация и eventual retirement старого cache в host-wide `default` builder без автоматического global prune;
