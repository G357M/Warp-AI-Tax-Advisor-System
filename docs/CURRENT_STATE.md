...
36. **The client cabinet has a fail-closed billing foundation.** A public backend catalog is the runtime authority for plan prices and feature codes. New checkout intents store the authenticated user, server-selected amount in integer tetri, currency, provider, status, expiry and a per-user idempotency key; a reused key cannot create a second order or switch plans. Manual invoice settlement can reference the exact checkout UUID and repeated admin settlement cannot duplicate the payment. The cabinet now shows plan period, quota, package comparison, provider readiness, pending request reference, confirmed payment history, conversations and support in RU/KA/EN. TBC and Bank of Georgia remain explicitly unavailable until merchant onboarding and verified adapters are complete; no credential-driven fallback can activate unfinished code. Automatic bank checkout, recurring charges and Business organizations/seats are still incomplete stages.
...
- выбор primary acquirer и подключение автоматизированной оплаты к готовому persisted-checkout contract;
