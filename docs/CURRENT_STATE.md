...
34. **Exact article links and citations are verified at the final public-response boundary.** A separate dry-run-first canary permits exactly nine loopback HTTP requests to `/api/v1/public/query`, three per RU/EN/KA, and refuses external targets or a different request ceiling. It covers Labour Code article 47, Tax Code article 168 and the homepage tax-appeal question grounded in Tax Code article 299. Every case requires a localized answer that names the controlling article, `exact_provision` evidence, the canonical Matsne article deep-link and the separately pinned publication URL in the serialized public source metadata. The deterministic appeal answer now cites Tax Code articles 296, 297 and 299 in RU/EN/KA. The accepted production run for `401ec58` passed 9/9 with all seven overall and per-language metrics at 1.0. Full answers and sources remain in a mode-600 operational report; only the aggregate allowlist is committed. Because public requests may invoke the provider, this canary remains manual and is not added to nightly automation.
