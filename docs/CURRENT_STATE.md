...
22. **Restore drills have a production-isolated contract.** A dry-run-first script fingerprints one explicit backup and requires the exact SHA-256 before execute. The restore uses a unique disposable pgvector/PostgreSQL container with no network, ports or mounted volumes; it fails on missing critical tables, insufficient corpus counts, orphan decision/chunk/link rows, unvalidated foreign keys or a missing vector extension. It records aggregate RPO/RTO evidence to a new mode-600 file and removes the temporary container on every exit. This makes the quarterly procedure reproducible; it is not evidence of a completed drill until the owner's real weekly backup has passed it.
...
24. **Duplicate review has an official-source automation layer.** A bounded stdlib-only verifier reads the protected review bundle, accepts only fixed InfoHub HTTPS document UUIDs and compares their current official public-API bodies, identity fields and metadata. It stores hashes, lengths and similarity evidence rather than legal text, performs no database/LLM work and cannot modify an expert worksheet or create a legal verdict. Identical official content becomes a batch-confirmation candidate; content differences, identity mismatches and failed fetches remain manual work.
