...
32. **Disk pressure is measured nightly without cleanup side effects.** Before ingest, a stdlib-only host audit checks root free/used space and aggregates Buildx JSON sizes for the isolated InfoHub builder and shared `default` builder. Defaults alert below 25 GB free, above 82% root usage, above 18 GB project cache or above the 60 GB legacy observation ceiling. Only aggregate JSON can reach Telegram; no cache description, prune, image removal or volume removal exists. The first inventory measured 54.26 GB legacy cache and attributed at least 38.427 GB to old InfoHub dependency/virtualenv layers. A separate dry-run-first exact-ID tool now revalidates immutable/non-shared metadata, binds execution to count/bytes/plan SHA-256 and verifies each selected ID after prune; it is intentionally absent from nightly automation.
...
- выполнить первый SHA-pinned exact-ID retirement четырёх уже атрибутированных старых InfoHub dependency records в host-wide `default` builder и зафиксировать post-check; nightly audit остаётся observe-only и global prune запрещён;
