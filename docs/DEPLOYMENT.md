...
После указанного cutoff удалите старый ключ из окружения и повторно проверьте,
что токен со старой подписью отклоняется:

```bash
./scripts/finalize_jwt_rotation.sh
```

Финализатор откажется работать до окончания grace window, проверит HTTP 401
для старой подписи до и после очистки и также автоматически восстановит
предыдущий `.env`, если backend или public health-check не пройдут.
